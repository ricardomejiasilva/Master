 ...
 * @package Catch_Mag
 */
$quantity     = get_theme_mod( 'catch_mag_service_number', 4 );
...
//Get valid number of posts
	...
	for ( $i = 1; $i <= $quantity; $i++ ) {
		...
		$post_id = get_theme_mod( 'catch_mag_service_cpt_' . $i );
		...
		<div class="hentry-inner">	
			<a class="post-thumbnail" href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>">
				...
				if ( $media_id = get_post_meta( $post->ID, 'ect-alt-featured-image', true ) ) {
					...
					echo wp_get_attachment_image( $media_id, 'catch-mag-testimonial', false,  array( 'title' => $title_attribute, 'alt' => $title_attribute ) );
				} elseif ( has_post_thumbnail() ) {
					the_post_thumbnail( 'catch-mag-testimonial' );
				} else {
					// Get the first image in page, returns false if there is no image.
					$first_image = catch_mag_get_first_image( get_the_ID(), 'catch-mag-testimonial', '' );
